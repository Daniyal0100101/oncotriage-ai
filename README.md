...
Current release: public prototype with live Evo 2 integration and synthetic-data ranking head.

> **Research / Educational Use Only - Not for Clinical Diagnosis or Treatment Decisions.**
...
- **Evo 2 foundation-model inference** - derives a Delta log-likelihood between reference and alternate DNA sequences as a proxy signal for sequence-model disruption.
- **A lightweight RandomForest ranking head** - maps Evo 2 scoring features to a *review-priority score* (0-1). It is currently trained on **synthetic** ClinVar-like data; real ClinVar validation is the next scientific milestone.
...
| `/forward` logits endpoint | Disabled (not exposed by hosted NVIDIA NIM) |
...
- **Single Variant Scoring** - Enter a BRCA1/BRCA2 variant in HGVS or positional notation and receive an Evo 2-derived review-priority score with a review-priority visualization and sequence-context visualization.
...
- **Result Provenance** - Every result includes inference mode, scoring path, model, and fallback status so you can see exactly what produced the output.
...
If a live API call fails due to an internal NVIDIA API error, the app catches the error gracefully, surfaces an inline warning, and falls back to demo estimates. Results are labeled **DEMO (fallback)**.

> **Note on `/forward`:** The hosted NVIDIA NIM platform does not expose the `/forward` logits endpoint; it returns an internal `StripedHyena has no attribute output_layer` error. The `/forward` path is disabled by default via `EVO2_USE_FORWARD_PASS=false`. Set it to `true` only on private NIM deployments that expose it.
...
The RandomForest ranking head is trained **offline** on synthetic ClinVar-like data using 13 features derived from Evo 2 log-likelihood outputs and sequence properties. Real ClinVar integration and validation are still required before any stronger scientific or operational claim.
...
**4. Configure environment** *(optional - the app runs in DEMO mode without a key)*
...
|---|---|---|---|
| `NVIDIA_API_KEY` | No | - | NVIDIA NIM API key (`nvapi-...`). Get one free at [build.nvidia.com](https://build.nvidia.com/arc/evo2-40b). The app runs in demo mode without it. |
...
### Generate Tab
![Generate Tab](docs/screenshots/generate.png)
...
- Variant coverage is limited to BRCA1 and BRCA2 contexts.
...
- There is no multi-user authentication or data isolation; do not host with shared access to sensitive sequences.
...
Copyright (c) Daniyal Asif. All rights reserved.
